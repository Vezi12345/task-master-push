...
class SmartRecruitersBrowserAdapter(BrowserAdapter):
    name = "smartrecruiters"

    def can_handle(self, url: str) -> bool:
        lowered = url.lower()
        return "smartrecruiters.com" in lowered
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                submit_btn = await page.query_selector(
                    'button[data-test="apply-submit"], button[type="submit"]'
                )
                if submit_btn:
                ...
                return BrowserResult(
                    success=False,
                    error="No submit button found on SmartRecruiters page",
                    requires_human_input=True,
                )
            except Exception as exc:
                return BrowserResult(
                    success=False,
                    error=f"SmartRecruiters browser error: {exc}",
                    requires_human_input=True,
                )
            finally:
                await browser.close()
